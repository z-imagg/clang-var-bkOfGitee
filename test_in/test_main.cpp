...
int main(int argc, char** argv){_VarDeclLs * _vdLs=_init_varLs_inFn__RtCxx("/fridaAnlzAp/clang-var/test_in/test_main.cpp", "main", 11, 32); /* 初始化函数变量列表, */
  ...
  std::for_each(ls.begin(), ls.end(), [](my_nsp::MyClass* k){_VarDeclLs * _vdLs=_init_varLs_inFn__RtCxx("/fridaAnlzAp/clang-var/test_in/test_main.cpp", "", 24, 61); /* 初始化函数变量列表, */ my_nsp::MyClass* idx=k; destroyVarLs_inFn__RtCxx(_vdLs); /* 销毁函数变量列表: */return k; });
  ...
  my_nsp::MyClass *(*lambdaFunc01)(my_nsp::UserEntity *) = [](my_nsp::UserEntity *uEk) {_VarDeclLs * _vdLs=_init_varLs_inFn__RtCxx("/fridaAnlzAp/clang-var/test_in/test_main.cpp", "", 27, 88); /* 初始化函数变量列表, */  my_nsp::UserEntity *uEi = uEk;   destroyVarLs_inFn__RtCxx(_vdLs); /* 销毁函数变量列表: */return (my_nsp::MyClass *)uEk; };
